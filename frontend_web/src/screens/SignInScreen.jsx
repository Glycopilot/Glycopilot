...
export default function SignInScreen({ navigation }) {
  ...
  const [medicalCenterPostalCode, setMedicalCenterPostalCode] = useState('');
  const [medicalCenterCity, setMedicalCenterCity]               = useState('');
  ...
  const handleSignIn = async () => {
    ...
    if (!specialty)            return toastError('Erreur', 'Veuillez sélectionner votre spécialité');
    if (!medicalCenterName)    return toastError('Erreur', 'Veuillez sélectionner votre structure');
    const addressCheck = await validateFrenchAddress({
      postalCode: medicalCenterPostalCode,
      city: medicalCenterCity,
      address: medicalCenterAddress,
    });
    if (!addressCheck.valid) return toastError('Erreur', addressCheck.error);
    ...
    try {
      await authService.register({
        ...
        role: 'DOCTOR', licenseNumber, specialty, medicalCenterName,
        medicalCenterAddress, medicalCenterPostalCode, medicalCenterCity,
      });
        ...
        <aside className="auth-aside">
          ...
          <div className="aside-body">
            ...
            <ul className="aside-steps">
              <li><span className="step-num step-num-done">✓</span><span>Compte créé avec succès</span></li>
            ...
            <div className="verification-card">
              ...
              <div className="verif-steps">
                <div className="vstep vstep-done">
                  <div className="vstep-dot vstep-dot-done"><Check size={16} strokeWidth={3} /></div>
              ...
              <div className="verif-notice">
                <Send size={18} className="notice-icon" />
    ...
    <div className="auth-root">
      ...
      <aside className="auth-aside">
        ...
        <div className="aside-body">
          ...
          <h1 className="aside-title">Rejoignez l'écosystème GlycoPilot</h1>
          <p className="aside-desc">Créez votre espace sécurisé en quelques instants. Accédez à des outils de pointe pour optimiser le suivi et la prise en charge de vos patients diabétiques.</p>
          <ul className="aside-steps">
            <li><span className="step-num">01</span><span>Créez votre profil praticien</span></li>
            <li><span className="step-num">02</span><span>Validez vos informations médicales</span></li>
            <li><span className="step-num">03</span><span>Accédez à votre tableau de bord</span></li>
          </ul>
        </div>
        <div className="aside-bottom">
          <span>Déjà inscrit ?</span>
          <button className="aside-link" onClick={goToLogin}>
            <span>Se connecter</span> <LogIn size={16} strokeWidth={2} />
          </button>
        </div>
      ...
      <main className="auth-main">
        <div className="auth-form-wrapper">
          ...
          <section className="form-section">
            ...
            <div className="row-2">
              <SelectField label="Spécialité" value={specialty} onChange={setSpecialty} options={DOCTOR_SPECIALTY_OPTIONS} icon={<Stethoscope size={16} strokeWidth={1.75} />} required />
              <SelectField label="Votre structure" value={medicalCenterName} onChange={setMedicalCenterName} options={DOCTOR_STRUCTURE_OPTIONS} icon={<Building2 size={16} strokeWidth={1.75} />} required />
            </div>
            <FrenchAddressFields
              postalCode={medicalCenterPostalCode}
              city={medicalCenterCity}
              address={medicalCenterAddress}
              onPostalCodeChange={setMedicalCenterPostalCode}
              onCityChange={setMedicalCenterCity}
              onAddressChange={setMedicalCenterAddress}
              variant="auth"
            />
          </section>
          ...
          <section className="form-section">
            ...
            <InputField
              ...
              icon={<Lock size={16} strokeWidth={1.75} />} type={showPassword ? 'text' : 'password'} placeholder="••••••••"
              rightElement={<button type="button" onClick={() => setShowPassword(!showPassword)} className="password-toggle" aria-label={showPassword ? 'Masquer' : 'Afficher'}>{showPassword ? <EyeOff size={16} strokeWidth={2} /> : <Eye size={16} strokeWidth={2} />}</button>}
            />
            ...
            <InputField
              ...
              icon={<Lock size={16} strokeWidth={1.75} />} type={showConfirmPassword ? 'text' : 'password'} placeholder="••••••••"
              rightElement={<button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="password-toggle" aria-label={showConfirmPassword ? 'Masquer' : 'Afficher'}>{showConfirmPassword ? <EyeOff size={16} strokeWidth={2} /> : <Eye size={16} strokeWidth={2} />}</button>}
            />
          </section>
