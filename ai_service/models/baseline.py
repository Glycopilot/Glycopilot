"""
Baseline model: Linear Regression per horizon.
Falls back to persistence model if artifacts not found.
"""
from __future__ import annotations

import os
import numpy as np
import joblib

from core.config import settings
from core.logger import get_logger

logger = get_logger(__name__)

HORIZON_INDICES = {15: 0, 30: 1, 60: 2}


def _risk(y_hat: float, threshold: float, direction: str, scale: float = 30.0) -> float:
    if direction == "hypo":
        return float(np.clip((threshold - y_hat) / scale, 0.0, 1.0))
    return float(np.clip((y_hat - threshold) / scale, 0.0, 1.0))


class BaselineModel:
    def __init__(self) -> None:
        self._models: dict[int, object] = {}
        self._loaded = False

    def load(self) -> None:
        base = settings.artifacts_dir
        all_ok = True
        for h in [15, 30, 60]:
            path = os.path.join(base, "baseline", f"lr_{h}_v1.0.pkl")
            if os.path.exists(path):
                self._models[h] = joblib.load(path)
            else:
                logger.warning("Baseline artifact not found: %s — using persistence", path)
                self._models[h] = None
                all_ok = False
        self._loaded = True
        if all_ok:
            logger.info("Baseline model loaded.")

    def is_loaded(self) -> bool:
        return self._loaded

    def predict(self, features: np.ndarray) -> dict:
        """
        features: (seq_len, N_features)
        Returns dict with y_hat/p10/p90/risk_hypo/risk_hyper for each horizon.
        """
        flat = features[-1]  # use last timestep for tabular prediction
        results = {}
        current_value = float(flat[0])

        for h in [15, 30, 60]:
            model = self._models.get(h)
            if model is not None:
                y_hat = float(model.predict(flat.reshape(1, -1))[0])
            else:
                y_hat = current_value  # persistence fallback

            p10 = y_hat - 15.0
            p90 = y_hat + 15.0
            results[h] = {
                "y_hat": round(y_hat, 2),
                "p10": round(p10, 2),
                "p90": round(p90, 2),
                "risk_hypo": _risk(y_hat, 70.0, "hypo"),
                "risk_hyper": _risk(y_hat, 180.0, "hyper"),
            }
        return results


baseline_model = BaselineModel()
