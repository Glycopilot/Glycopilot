...
class CareTeamIntegrationTests(TestCase):
    ...
    def test_add_family_errors(self):
        """Test les erreurs lors de l'ajout d'un membre de la famille."""
        patient_identity = UserIdentity.objects.create(
            first_name="Paul", last_name="Patient", phone_number="0600000010"
        )
        User.objects.create_user(
            email="paul_err@test.com",
            password="pass123",
            user_identity=patient_identity,
        )
        Profile.objects.create(user=patient_identity, role=self.patient_role)
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self._token_for('paul_err@test.com', 'pass123')}"
        )
        # Rôle invalide
        response = self.client.post(
            "/api/doctors/care-team/add-family/",
            {"first_name": "Marie", "last_name": "Fam", "role": "INVALID_ROLE"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("Rôle invalide", response.data.get("error", ""))

    def test_invite_doctor_errors(self):
        """Test les erreurs lors de l'invitation d'un médecin."""
        patient_identity = UserIdentity.objects.create(
            first_name="Paul", last_name="Patient", phone_number="0600000011"
        )
        User.objects.create_user(
            email="paul_doc_err@test.com",
            password="pass123",
            user_identity=patient_identity,
        )
        Profile.objects.create(user=patient_identity, role=self.patient_role)
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self._token_for('paul_doc_err@test.com', 'pass123')}"
        )
        ...
        response = self.client.post(
            "/api/doctors/care-team/invite-doctor/",
            {"email": "paul_doc_err@test.com", "role": "REFERENT_DOCTOR"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response = self.client.post(
            "/api/doctors/care-team/invite-doctor/",
            {"email": "nobody@test.com", "role": "REFERENT_DOCTOR"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_add_patient_errors(self):
        """Test les erreurs lors de l'ajout d'un patient par un médecin."""
        patient_identity = UserIdentity.objects.create(
            first_name="Paul", last_name="Patient", phone_number="0600000012"
        )
        User.objects.create_user(
            email="paul_pat_err@test.com",
            password="pass123",
            user_identity=patient_identity,
        )
        Profile.objects.create(user=patient_identity, role=self.patient_role)

        # Patient essaie d'ajouter un patient (seul un docteur peut)
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self._token_for('paul_pat_err@test.com', 'pass123')}"
        )
        response = self.client.post(
    ...
    def test_add_patient_unverified_doctor(self):
        """Un docteur non validé tente d'ajouter un patient existant."""
        # 1. Créer le docteur en tant que VERIFIED pour obtenir le token (car le login est bloqué sinon)
        doctor_identity = UserIdentity.objects.create(
            first_name="Doc", last_name="Unv", phone_number="0600000013"
        )
        User.objects.create_user(
            email="doc_unv_err@test.com",
            password="pass123",
            user_identity=doctor_identity,
        )
        doc_profile = Profile.objects.create(
        ...
        doc_prof.license_number = "LIC-ERR"
        doc_prof.save()

        token = self._token_for("doc_unv_err@test.com", "pass123")

        # 2. Rétrograder le docteur à PENDING
        doc_prof.verification_status = self.pending_status
        doc_prof.save()

        # 3. Essayer d'ajouter un patient avec le token valide
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        response = self.client.post(
            "/api/doctors/care-team/add-patient/",
            {"email": "somepatient@test.com"},
        )
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_accept_invitation_errors(self):
        """Erreurs lors de l'acceptation d'une invitation."""
        patient_identity = UserIdentity.objects.create(
            first_name="Paul", last_name="Patient", phone_number="0600000014"
        )
        User.objects.create_user(
            email="paul_acc_err@test.com",
            password="pass123",
            user_identity=patient_identity,
        )
        Profile.objects.create(user=patient_identity, role=self.patient_role)
        self.client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {self._token_for('paul_acc_err@test.com', 'pass123')}"
        )

        # Invitation inexistante
        response = self.client.post(
            "/api/doctors/care-team/accept-invitation/",
            {"id_team_member": "00000000-0000-0000-0000-000000000000"},
        )
