...
class CareTeamIntegrationTests(TestCase):
    ...
    def test_patient_invite_unverified_doctor_returns_not_available(self):
        ...
        patient_identity = UserIdentity.objects.create(first_name="Paul", last_name="Patient", phone_number="0600000001")
        patient_account = User.objects.create_user(email="paul@test.com", password="pass123", user_identity=patient_identity)
        ...
        doctor_identity = UserIdentity.objects.create(first_name="Doc", last_name="Unverified", phone_number="0600000002")
        User.objects.create_user(email="doc_unverified@test.com", password="pass123", user_identity=doctor_identity)
        doc_profile = Profile.objects.create(user=doctor_identity, role=self.doctor_role)
        doc_prof = doc_profile.doctor_profile
        doc_prof.verification_status = self.pending_status
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('paul@test.com', 'pass123')}")
        response = self.client.post(
        ...
        self.assertFalse(
            PatientCareTeam.objects.filter(patient_profile=p_profile.patient_profile, member_profile=doc_profile).exists()
        )
    ...
    def test_admin_validates_doctor_then_patient_can_invite(self):
        ...
        # Patient
        patient_identity = UserIdentity.objects.create(first_name="Alice", last_name="Patient", phone_number="0611111111")
        User.objects.create_user(email="alice@test.com", password="pass123", user_identity=patient_identity)
        p_profile = Profile.objects.create(user=patient_identity, role=self.patient_role)

        # Docteur PENDING
        doctor_identity = UserIdentity.objects.create(first_name="Bob", last_name="Doctor", phone_number="0622222222")
        User.objects.create_user(email="bob_doctor@test.com", password="pass123", user_identity=doctor_identity)
        doc_profile = Profile.objects.create(user=doctor_identity, role=self.doctor_role)
        doc_prof = doc_profile.doctor_profile
        doc_prof.verification_status = self.pending_status
        ...
        # Admin (superadmin)
        admin_identity = UserIdentity.objects.create(first_name="Admin", last_name="Super")
        admin_account = User.objects.create_user(email="admin@test.com", password="admin123", user_identity=admin_identity)
        admin_account.is_staff = True
        admin_account.is_superuser = True
        admin_account.save()
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('admin@test.com', 'admin123')}")
        response = self.client.post(f"/api/doctors/verification/{doc_prof.doctor_id}/accept/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('alice@test.com', 'pass123')}")
        response = self.client.post(
            "/api/doctors/care-team/invite-doctor/",
    ...
    def test_unverified_doctor_cannot_add_patient(self):
        ...
        doctor_identity = UserIdentity.objects.create(first_name="Unv", last_name="Doc", phone_number="0633333333")
        User.objects.create_user(email="unv_doc@test.com", password="pass123", user_identity=doctor_identity)
        doc_profile = Profile.objects.create(user=doctor_identity, role=self.doctor_role)
        doc_profile.doctor_profile.verification_status = self.pending_status
        doc_profile.doctor_profile.license_number = "LIC-UNV"
        doc_profile.doctor_profile.save()

        patient_identity = UserIdentity.objects.create(first_name="Pat", last_name="Two", phone_number="0644444444")
    ...
    def test_verified_doctor_can_add_patient(self):
        ...
        doctor_identity = UserIdentity.objects.create(first_name="Verified", last_name="Doc", phone_number="0655555555")
        User.objects.create_user(email="verified_doc@test.com", password="pass123", user_identity=doctor_identity)
        doc_profile = Profile.objects.create(user=doctor_identity, role=self.doctor_role)
        doc_profile.doctor_profile.verification_status = self.verified_status
        doc_profile.doctor_profile.license_number = "LIC-VERIFIED"
        doc_profile.doctor_profile.save()

        patient_identity = UserIdentity.objects.create(first_name="NewPat", last_name="Three", phone_number="0666666666")
        User.objects.create_user(email="newpat@test.com", password="pass123", user_identity=patient_identity)
        Profile.objects.create(user=patient_identity, role=self.patient_role)

        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('verified_doc@test.com', 'pass123')}")
        response = self.client.post("/api/doctors/care-team/add-patient/", {"email": "newpat@test.com"})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertIn("message", response.data)
    ...
    def test_full_flow_patient_invitations_and_doctor_add_patient(self):
        ...
        patient_identity = UserIdentity.objects.create(first_name="Full", last_name="Patient", phone_number="0677777777")
        User.objects.create_user(email="full_patient@test.com", password="pass123", user_identity=patient_identity)
        p_profile = Profile.objects.create(user=patient_identity, role=self.patient_role)
        patient_profile = p_profile.patient_profile

        # 2. Patient ajoute un membre famille
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('full_patient@test.com', 'pass123')}")
        response = self.client.post(
            "/api/doctors/care-team/add-family/",
            {"first_name": "Marie", "last_name": "Famille", "role": "FAMILY", "relation_type": "Épouse"},
        )
        ...
        # 3. Docteur PENDING
        doctor_identity = UserIdentity.objects.create(first_name="Full", last_name="Doc", phone_number="0688888888")
        User.objects.create_user(email="full_doc@test.com", password="pass123", user_identity=doctor_identity)
        doc_profile = Profile.objects.create(user=doctor_identity, role=self.doctor_role)
        doc_prof = doc_profile.doctor_profile
        doc_prof.verification_status = self.pending_status
        ...
        admin_identity = UserIdentity.objects.create(first_name="Super", last_name="Admin")
        admin_account = User.objects.create_user(email="super@test.com", password="admin123", user_identity=admin_identity)
        admin_account.is_staff = True
        admin_account.is_superuser = True
        admin_account.save()
        Profile.objects.create(user=admin_identity, role=Role.objects.get(name="SUPERADMIN"))

        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('super@test.com', 'admin123')}")
        response = self.client.post(f"/api/doctors/verification/{doc_prof.doctor_id}/accept/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(
            PatientCareTeam.objects.filter(patient_profile=patient_profile, member_profile=doc_profile).exists()
        )

        # 7. Docteur ajoute un autre patient (deuxième patient)
        patient2_identity = UserIdentity.objects.create(first_name="Second", last_name="Patient", phone_number="0699999999")
        User.objects.create_user(email="second_patient@test.com", password="pass123", user_identity=patient2_identity)
        Profile.objects.create(user=patient2_identity, role=self.patient_role)

        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self._token_for('full_doc@test.com', 'pass123')}")
        response = self.client.post("/api/doctors/care-team/add-patient/", {"email": "second_patient@test.com"})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
