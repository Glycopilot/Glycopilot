...
class CareTeamViewSet(viewsets.ViewSet):
    ...
    def _get_patient_team(self, patient_profile):
        ...
        data = {
            ...
            "pending_doctor_invites": PatientCareTeamSerializer(
                pending_doctor_invites, many=True
            ).data,
    ...
    def remove_member(self, request):
        ...
        if not patient_role_profile or not hasattr(
            patient_role_profile, "patient_profile"
        ):
            return Response(
                {"error": "Seuls les patients peuvent retirer un membre."}, status=403
            )
