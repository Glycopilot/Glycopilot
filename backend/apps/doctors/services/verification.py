...
class DoctorVerificationService:
    ...
    def verify_license(license_number: str, specialty: str = None) -> bool:
        ...
        if not api_url:
            logger.warning(
                "LICENCE_VERIFICATION_API is not set. Skipping real verification."
            )
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    logger.warning(
                        f"Verification API returned non-JSON response for license {license_number}"
                    )
                    return False
            else:
                logger.warning(
                    f"Verification API returned status {response.status_code} for license {license_number}"
                )
                return False
