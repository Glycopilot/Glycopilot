...
def send_due_medication_reminders() -> dict:
    ...
    window_start = (
        (local_now - timedelta(seconds=REMINDER_WINDOW_SECONDS))
        .time()
    ...
    due_intakes = MedicationIntake.objects.filter(
    ...
    for intake in due_intakes:
        ...
        try:
            ...
            if result.get("success"):
                stats["sent"] += 1
                logger.info(
                    "[REMINDER] sent intake=%s at=%s", intake.id, intake.scheduled_time
                )
            else:
                stats["skipped"] += 1
                logger.info(
                    "[REMINDER] skipped intake=%s reason=%s",
                    intake.id,
                    result.get("error"),
                )
    ...
    logger.info(
        "[REMINDER] done sent=%s skipped=%s errors=%s",
        stats["sent"],
        stats["skipped"],
        stats["errors"],
    )
