...
class UserMedicationViewSet(viewsets.ModelViewSet):
    ...
    def today(self, request):
        ...
        active_meds = (
            ...
            .filter(models_end_date_filter(today))
        )
...
class MedicationScheduleViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        medication_id = self.request.data.get("medication_id") or self.request.data.get(
            "user_medication"
        )
        user_med = UserMedication.objects.get(pk=medication_id, user=self.request.user)
