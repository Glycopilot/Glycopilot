        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    recommended_duration = int(row['recommended_duration']) if row.get('recommended_duration') else None
                    calories_burned = int(row['calories_burned']) if row.get('calories_burned') else None
                    sugar_used = float(row['sugar_used']) if row.get('sugar_used') else None
                    ...
                    self.stdout.write(self.style.ERROR(f"Error processing activity ID {row.get('id')}: {e}"))
            
            self.stdout.write(self.style.SUCCESS(f"Activities: {count_created} created, {count_updated} updated."))
