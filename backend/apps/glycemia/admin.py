...
class PersonalModelApprovalAdmin(admin.ModelAdmin):
    list_display = ("patient_id", "version", "status", "mae_15", "mae_30", "mae_60", "trained_at", "approved_by")
    list_filter = ("status", "version")
    search_fields = ("patient_id",)
    readonly_fields = (
        "id", "patient_id", "version", "mae_15", "mae_30", "mae_60",
        "trained_at", "approved_at", "approved_by",
    )
    ...
    def approve_models(self, request, queryset):
        ...
        for obj in queryset.filter(status=PersonalModelApproval.STATUS_PENDING):
            try:
                _call_ai_service(f"/finetune/{obj.patient_id}/approve?version={obj.version}")
                ...
                self.message_user(request, f"Erreur patient {obj.patient_id}: {exc}", level="error")
        if done:
    ...
    def reject_models(self, request, queryset):
        ...
        for obj in queryset.filter(status=PersonalModelApproval.STATUS_PENDING):
            try:
                _call_ai_service(f"/finetune/{obj.patient_id}/reject?version={obj.version}")
