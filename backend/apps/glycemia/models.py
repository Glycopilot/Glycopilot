...
class GlycemiaHisto(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="glycemia_histo")
...
class GlycemiaDataIA(models.Model):
    id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="glycemia_ai")
    measured_at = models.DateTimeField(db_index=True)
    value = models.FloatField()
