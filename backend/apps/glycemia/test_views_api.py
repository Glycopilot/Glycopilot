...
class GlycemiaCurrentViewTests(TestCase):
    def setUp(self):
...
class GlycemiaRangeViewTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(email="gly-range@test.com", password="pass123")
        self.client.force_authenticate(user=self.user)

    def test_range_returns_history_for_default_period(self):
    ...
    def test_range_returns_empty_entries_when_no_data(self):
        response = self.client.get("/api/glycemia/range/")
...
class ManualReadingCreateTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(email="manual@test.com", password="pass123")
        self.client.force_authenticate(user=self.user)

    def test_create_manual_reading_stores_in_both_tables(self):
        payload = {
        ...
        self.assertTrue(GlycemiaHisto.objects.filter(user=self.user, value=98.0).exists())
