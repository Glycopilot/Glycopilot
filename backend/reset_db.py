...
def reset_database():
    ...
    from apps.doctors.models.status import InvitationStatus, VerificationStatus
    ...
    if current_env != "production":
        ...
        if not AuthAccount.objects.filter(email="patient@example.com").exists():
            ...
            AuthAccount.objects.create_user(
                email="patient@example.com", password="StrongPass123!", user_identity=u
            )
        ...
        if not AuthAccount.objects.filter(email="doctor@example.com").exists():
            ...
            AuthAccount.objects.create_user(
                email="doctor@example.com", password="StrongPass123!", user_identity=u
            )
